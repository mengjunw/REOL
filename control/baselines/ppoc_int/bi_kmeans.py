...
def load_data(path):
	...
	#嵌套列表去重
	data_set = [list(t) for t in set(tuple(_) for _ in data_set)]
	return data_set
...
def get_option(currentcell, centroid):
	min_dis = []
	centroid = array(centroid)
	for id in centroid:
		dis = 0
		for i in range(len(id)):
			dis += (id[i]-currentcell[i])**2
		min_dis.append(dis)
	return min_dis.index(min(min_dis))
...
# 	lst = list()
# 	for i in range(5):
# 		data_mat = mat(load_data("/data/test_data_HalfCheetahDir-v1_2.txt"))
# 		centroid, cluster_assment = bi_kMeans(data_mat, 2)
