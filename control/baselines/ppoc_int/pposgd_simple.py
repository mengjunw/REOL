...
def traj_segment_generator(pi, env, horizon, stochastic, num_options,saves,results,rewbuffer,dc,epoch,seed,plots, w_intfc,switch,aggregate):
    t = 0
    ...
    aggregates = np.zeros(horizon, 'float32')
    ...
    while True:
        ...
        if aggregate == 1:
            ob_ = copy.deepcopy(ob)
            if hasattr(env, 'NAME'):
                ob_ = np.round(ob_, decimals=2)
                dim = ob_.shape[0]
                err = np.zeros(dim, "float32")
                sampling_op_aggregate_list = []
                sampling_num = int(math.ceil(dim / 5) + 1)
                sampling_dim_list = [np.random.randint(0, dim) for i in range(sampling_num)]
                for i in sampling_dim_list:
                    err[i] += 0.05
                    sampling_ob = ob_ + err
                    sampling_op_prob, _ = pi.get_intvals(sampling_ob)
                    sampling_op_aggregate_list.append(np.abs(sampling_op_prob[0][option] - op_prob[0][option]))
                    err[i] -= 0.05 * 2
                    sampling_ob = ob_ + err
                    sampling_op_prob, _ = pi.get_intvals(sampling_ob)
                    sampling_op_aggregate_list.append(np.abs(sampling_op_prob[0][option] - op_prob[0][option]))
                # mean_sampling_op_aggregate = np.mean(sampling_op_aggregate_list)
                op_aggregate = np.var(sampling_op_aggregate_list)
                # print(op_aggregate)
            else:
                ob_ = np.round(ob_, decimals=2)
                dim = ob_.shape[0]
                ...
                for i in sampling_dim_list:
                    ...
                    sampling_op_prob, _ = pi.get_intvals(sampling_ob)
                    sampling_op_aggregate_list.append(np.abs(sampling_op_prob[0][option] - op_prob[0][option]))
                # print(sampling_op_aggregate_list)
                op_aggregate = np.var(sampling_op_aggregate_list)
                # print(op_aggregate)
        ...
        if t > 0 and t % horizon == 0:
            ...
            op_usages = copy.deepcopy(opts)

            if aggregate == 1:
                opts_mask = np.unique(opts)
                tmp = {}
                for v in opts_mask:
                    tmp[v] = np.sum(opts == v)
                tmp_prob = {}
                for i in tmp:
                    tmp_prob[i] = tmp[i]/horizon

                num_of_options_used = list(set(op_usages))
                for i in num_of_options_used:
                    op_usages = np.where(op_usages==i, tmp_prob[i], op_usages)
            ...
            yield {"ob" : obs, "rew" : rews, "realrew": realrews, "vpred" : vpreds, "op_vpred": op_vpreds, "new" : news,
                   ...
                   "aggregates":aggregates, "op_usages":op_usages}
            ...
            if iters_so_far == 140 and hasattr(env, 'NAME') and env.NAME == 'TMaze':
                env.reset_count()
            ###### Switching Goal ##########
...
def learn(env, policy_func, *,
        ...
        epoch=0,
        seed=1,
        dc=0,plots=False,w_intfc=True,switch=False,intlr=1e-4,piolr=1e-4,fewshot=False,k=0.,aggregate=1
        ):
    ...
    aggregates = tf.placeholder(dtype=tf.float32, shape=None)
    ...
    # if aggregate == 1:
    #     term_loss += 0.01*(tf.reduce_mean(aggregates)-aggregates)
    ...
    op_usages = tf.placeholder(dtype=tf.float32, shape=None)
    ...
    if aggregate == 1:
        op_loss += (100*aggregates)
        #-0.05*((0.5-op_usages) * tf.log(tf.clip_by_value(np.abs(0.5-op_usages), 1e-20, 1.0)))
        # op_loss += 0.005*(-op_usages * tf.log(tf.clip_by_value(op_usages, 1e-20, 1.0)))
    ...
    termgrad = U.function([ob, option, term_adv,aggregates], [U.flatgrad(term_loss, var_list)]) # Since we will use a different step size.
    opgrad = U.function([ob, option, betas, op_adv, intfc, activated_options, aggregates, op_usages], [U.flatgrad(op_loss, var_list)]) # Since we will use a different step size.
    ...
    if saves:
        ...
        if w_intfc: 
            results = open(directory_res + gamename +'intfc{}_lr_{}_intlr{}_piolr{}_k{}_seed{}_aggregate{}'.format(int(w_intfc),optim_stepsize,intlr,piolr,k,seed,aggregate) + '.csv','w')
        else:
            results = open(directory_res + gamename +'intfc{}_lr_{}_intlr{}_piolr{}_k{}_seed{}_aggregate{}'.format(int(w_intfc),optim_stepsize,intlr,piolr,k,seed,aggregate) + '.csv','w')
    ...
    # ----------------------------------------
    seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, num_options=num_options,saves=saves,results=results,rewbuffer=rewbuffer,dc=dc,epoch=epoch,seed=seed,plots=plots,w_intfc=w_intfc,switch=switch, aggregate=aggregate)
    ...
    while True:
        ...
        ob, ac, opts, atarg, tdlamret, aggregates, op_usages = seg["ob"], seg["ac"], seg["opts"], seg["adv"], seg["tdlamret"], seg["aggregates"], seg["op_usages"]
        ...
        if iters_so_far<150 or not fewshot:
            termg = termgrad(seg["ob"], seg['opts'], seg["op_adv"], seg["aggregates"])[0]
        ...
        opgrads = opgrad(seg['ob'],seg['opts'], seg["last_betas"], seg["op_adv"], seg["intfc"], seg["activated_options"], seg["aggregates"], seg["op_usages"])[0]
        ...
        if saves:
            ...
            for i in range(num_options):
                out += ",{}"
            out+="\n"
            info = [iters_so_far, episodes_so_far, np.mean(lenbuffer), np.mean(rewbuffer), entropy_of_sampling_state, number_of_options_changed]
            for i in range(num_options):
