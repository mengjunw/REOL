...
import csv
from baselines.ppoc_int import bi_kmeans

def get_single_representation_erasure(pi, ob, num_options):
    ob_dim = len(ob)
    seg = {}
    seg["ob"] = np.array([ob for _ in range(ob_dim+1)])
    seg["ob"][0] = ob
    for i in range(1, ob_dim+1):
        mask_ob = copy.deepcopy(ob)
        mask_ob[i-1] = 0
        seg["ob"][i] = mask_ob
    seg["vpreds"] = np.array([num_options for _ in range(ob_dim+1)])
    seg["vpreds"] = pi.get_vpreds(seg["ob"])

    max_vpred = np.sum(seg["vpreds"][0])
    max_vpred_index = 0
    for index, vpred in enumerate(seg["vpreds"]):
        current_vpred = np.sum(vpred)
        if(current_vpred > max_vpred):
            max_vpred = current_vpred
            max_vpred_index = index

    return seg["ob"][max_vpred_index]

def get_complex_representation_erasure(pi, ob, num_options, w_intfc, u):
    ob_dim = np.size(ob)
    seg = {}
    seg["ob"] = np.array([ob for _ in range(ob_dim+1)])
    # seg["vpreds"] = np.array([num_options for _ in range(ob_dim + 1)])
    seg["td_op_prob"] = np.zeros(ob_dim+1, "float32")
    seg["td_op_vpred"] = np.zeros(ob_dim+1, "float32")
    seg["ob"][0] = ob
    for i in range(1, ob_dim+1):
        mask_ob = copy.deepcopy(ob)
        mask_ob[i-1] = 0
    ...
    # seg["vpreds"] = pi.get_vpreds(seg["ob"])
    vpreds, op_vpreds, _, _, op_probs, int_func = pi.get_allvpreds(seg["ob"], ob)
    if w_intfc:
        op_probs = op_probs * int_func / np.sum(op_probs * int_func,axis=1)[:,None]
    standard_op_prob = op_probs[0]
    for index, op_prob in enumerate(op_probs):
        td_op_prob = np.array(op_prob) - np.array(standard_op_prob)
        seg["td_op_prob"][index] = np.sum(np.multiply(td_op_prob, td_op_prob))

    standard_op_vpred = op_vpreds[0]
    for index, op_vrep in enumerate(op_vpreds):
        seg["td_op_vpred"][index] = op_vrep - standard_op_vpred

    seg["influence"] = np.multiply(seg["td_op_prob"], seg["td_op_vpred"])
    sorted_influece_index = sorted(range(len(seg["influence"])), key=lambda k: seg["influence"][k], reverse=True)

    num = 0
    all_standard_influence = 0
    for index in sorted_influece_index:
        if seg["influence"][index] > 0:
            num += 1
            all_standard_influence += seg["influence"][index]
        else:
            break
    if(num):
        mean_standard_influence = all_standard_influence/num
        for index in sorted_influece_index:
            if seg["influence"][index] > mean_standard_influence and seg["influence"][index] > u*standard_op_vpred:
                ...
                break
    return seg["ob"][0], seg["td_op_prob"][1:], seg["td_op_vpred"][1:], seg["influence"][1:]

def save_influence_data(seg, env_id, switch):
    with open("res/" + str(env_id) + "/" + str(env_id) + "_" + str(switch) +"_op_probs_variance.csv", 'a') as f1:
        writer = csv.writer(f1)
        writer.writerow(seg["op_probs_variance"].mean(axis=0))
    with open("res/" + str(env_id) + "/" + str(env_id) + "_" + str(switch) +"_op_vpreds_adv.csv", 'a') as f2:
        writer = csv.writer(f2)
        writer.writerow(seg["op_vpreds_adv"].mean(axis=0))
    with open("res/" + str(env_id) + "/" + str(env_id) + "_" + str(switch) +"_influence.csv", 'a') as f3:
        writer = csv.writer(f3)
        writer.writerow(seg["influence"].mean(axis=0))

def traj_segment_generator(pi, env, horizon, stochastic, num_options,saves,results,rewbuffer,dc,epoch,seed,plots, w_intfc, switch, pretraining, u):
    t = 0
    ...
    if hasattr(env,'NAME')and env.NAME=='AntWalls':
        env_id = env.NAME
        switch_iter = 240
    elif hasattr(env,'NAME')and env.NAME=='TMaze':
        env_id = env.NAME
        switch_iter = 150
    ...
    op_probs_variance = np.zeros((horizon, ob_dim), 'float32')
    op_vpreds_adv = np.zeros((horizon, ob_dim), 'float32')
    influence = np.zeros((horizon, ob_dim), 'float32')

    last_options=np.zeros(horizon, 'int32')
    acs = np.array([ac for _ in range(horizon)])
    prevacs = acs.copy()

    dirname = "res/" + str(env_id) + "/"
    if not os.path.exists(dirname):
        os.makedirs(dirname)


    ob, op_probs_variance[t % horizon], op_vpreds_adv[t % horizon], influence[t % horizon] = get_complex_representation_erasure(pi, ob, num_options, w_intfc, u)
    ...
    while True:
        ...
        if t > 0 and t % horizon == 0:
            ...
            yield {"ob" : obs, "rew" : rews, "realrew": realrews, "vpred" : vpreds, "op_vpred": op_vpreds, "new" : news,
                   ...
                   "op_probs_variance":op_probs_variance, "op_vpreds_adv": op_vpreds_adv, "influence": influence}
            ...
            if iters_so_far == switch_iter and switch:
                ...
                if hasattr(env, 'NAME') and env.NAME == 'TMaze':  # Switch the goal for TMaze
                    ...
                    print("New Goal:", env.target)
                elif hasattr(env, 'NAME') and env.NAME == 'AntWalls':  # Switch the goal for AntWalls
...
def information_entropy_of_state(ops_probs):
    entropy_list = []
    for ops_prob in ops_probs:
        entropy_list.append(-sum(i*math.log2(i) for i in ops_prob))
    entropy_of_sampling_state = sum(entropy_list)
    return entropy_of_sampling_state

def get_number_of_options_changed(pi, obs, last_opts):
    number_of_options_changed = 0
    for i in range(len(last_opts)):
        option,active_options_t = pi.get_option(obs[i])
        if option != last_opts[i]:
            number_of_options_changed += 1
    return number_of_options_changed
...
def learn(env, policy_func, *,
        ...
        epoch=0,
        seed=1,
        dc=0,plots=False,w_intfc=True,switch=False,intlr=1e-4,piolr=1e-4,fewshot=False,k=0.,u=0.01
        ):
    ...
    ### More book-kepping
    if saves:
        directory_res = "res/opt{}/".format(num_options)
        if not os.path.exists(directory_res):
            os.makedirs(directory_res)
        if w_intfc:
            results = open(
                directory_res + gamename + 'intfc{}_lr_{}_intlr{}_piolr{}_seed{}_u{}_reol'.format(int(w_intfc),
                                                                                                 optim_stepsize,
                                                                                                 intlr, piolr,
                                                                                                 seed, u) + '.csv',
                'w')
            ...
            results = open(
                directory_res + gamename + 'intfc{}_lr_{}_intlr{}_piolr{}_seed{}_u{}_reol'.format(int(w_intfc),
                                                                                                 ...
                                                                                                 seed, u) + '.csv',
                'w')
        out = 'epoch,EpisodesSoFar,EpLenMean,avg_reward,entropy_of_sampling_state,number_of_options_changed, op_probs'
    ...
    # ----------------------------------------
    seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, num_options=num_options,saves=saves,results=results,rewbuffer=rewbuffer,dc=dc,epoch=epoch,seed=seed,plots=plots,w_intfc=w_intfc,switch=switch, pretraining=False, u=0.01)
    ...
    while True:
        ...
        if((iters_so_far+1)%30 == 0):
            save_influence_data(seg, env_id, 0)
        ...
        number_of_options_changed = get_number_of_options_changed(pi, seg['ob'], seg['opts'])
        entropy_of_sampling_state = information_entropy_of_state(seg['op_probs'])
        ### Book keeping
        if saves:
            out = "{},{},{},{},{},{}"
            for i in range(num_options):
                out += ",{}"
            out += "\n"
            info = [iters_so_far, episodes_so_far, np.mean(lenbuffer), np.mean(rewbuffer), entropy_of_sampling_state,
                    number_of_options_changed]
            for i in range(num_options):
                info.append(np.mean(np.array(seg['op_probs']), axis=0)[i])
