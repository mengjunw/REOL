...
def train(env_id, num_timesteps, seed, num_options,app, saves ,wsaves, epoch,dc,plots,w_intfc,switch,mainlr,intlr,piolr,fewshot,k,aggregate):
    ...
    pposgd_simple.learn(env, policy_fn, 
            ...
            w_intfc=w_intfc,switch=switch,intlr=intlr,piolr=piolr,fewshot=fewshot,k=k,aggregate=aggregate
        )
    env.close()
...
def main():
    ...
    parser.add_argument('--timesteps', help='number of timesteps', type=int, default=1.5e6)
    ...
    parser.add_argument('--saves', help='Save the returns at each iteration', dest='saves', action='store_true', default=True)
    ...
    parser.add_argument('--switch', help='Switch task after 150 iterations', dest='switch', action='store_true', default=True)
    ...
    parser.add_argument('--aggregate', help="enables aggregate functions", type=int, default=0)
    ...
    train(args.env, num_timesteps=args.timesteps, seed=args.seed, num_options=args.opt, app=args.app,
     ...
     w_intfc=args.w_intfc,switch=args.switch,mainlr=args.mainlr,intlr=args.intlr,piolr=args.piolr,fewshot=args.fewshot,k=args.k, aggregate=args.aggregate)
